...
void gather_netadapters(std::ostream& response, const std::string& static_labels) {
    ...
    if (netadapter_count == -1) {
        std::cout << "netadapter count returned error " << errno << std::endl;
        perror("Error calling netadapter");
        return;
    }
	perfstat_netadapter_t *netadapters = new perfstat_netadapter_t[netadapter_count];
