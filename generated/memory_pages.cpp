...
void gather_memory_pages(std::ostream& response, const std::string& static_labels) {
    ...
    if (memory_page_count == -1) {
        std::cout << "memory_page count returned error " << errno << std::endl;
        perror("Error calling memory_page");
        return;
    }
	perfstat_memory_page_t *memory_pages = new perfstat_memory_page_t[memory_page_count];
