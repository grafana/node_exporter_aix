...
void gather_diskpaths(std::ostream& response, const std::string& static_labels) {
    ...
    if (diskpath_count == -1) {
        std::cout << "diskpath count returned error " << errno << std::endl;
        perror("Error calling diskpath");
        return;
    }
	perfstat_diskpath_t *diskpaths = new perfstat_diskpath_t[diskpath_count];
