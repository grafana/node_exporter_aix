...
void gather_netbuffers(std::ostream& response, const std::string& static_labels) {
    ...
    if (netbuffer_count == -1) {
        std::cout << "netbuffer count returned error " << errno << std::endl;
        perror("Error calling netbuffer");
        return;
    }
	perfstat_netbuffer_t *netbuffers = new perfstat_netbuffer_t[netbuffer_count];
